...
const ProductDetails = props => {
  ...
  useEffect(() => {
    superagent.get(`https://dunia-store-server.herokuapp.com/product/${id}`)
    .then(response => {
  ...
  return (
    ...
    <section className="prod">
      ...
      <Card className={classes.root} key={item.id} id="card1">
        <CardActionArea>
          <CardMedia
      ...
      <Card className={classes.root} id="card1">
        <Button id="buy" style={style} className={classes.button} size="large" fullWidth color="primary" onClick={() => props.addToCart(item.id, item)}>
              ADD TO CART
        </Button>
      </Card>
      ...
      <Card className={classes.root} id="card2">
        <Grid container>
          <Grid item sm={4}>
            <Card>
